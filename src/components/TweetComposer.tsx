...
const TweetComposer = ({ onTweetPosted }: any) => {
  const { user }: any = useAuth(); // useAuth-ilirundhu user data-vai edukkirom
  ...
  const handleSubmit = async (e: any) => {
    ...
    setIsLoading(true);
    try {
      // Backend-kku anuppa vendiya data-vai ippo fix panniyullaen
      const tweetdata = {
        userEmail: user?.email,
        postText: content,
        image: imageurl,
        // Backend 'index.js'-il neenga 'userName' matrum 'avatar' ethirpaarkireergal
        userName: user?.displayName || "Hamrutha Saravanan",
        avatar: user?.photoURL || user?.avatar || "", // Avatar-ai ingae add panniyaachu
      };

      console.log("Sending data to backend:", tweetdata);
      ...
      // Post aanavudan values-ai reset seigirom
      setContent("");
      setimageurl("");
    } catch (error) {
        ...
        <div className="flex space-x-4">
          ...
          <div className="flex-1">
            <form onSubmit={handleSubmit}>
              ...
              {imageurl && (
                <div className="relative mb-3 rounded-2xl overflow-hidden border border-gray-800">
                   <img src={imageurl} alt="Upload preview" className="w-full h-auto max-h-80 object-cover" />
                   <Button 
                    variant="destructive" 
                    size="sm" 
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="flex items-center space-x-4 text-blue-400">
                  ...
                  <Button type="button" variant="ghost" size="sm" className="p-2 rounded-full hover:bg-blue-900/20">
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="relative w-8 h-8">
                          <svg className="w-8 h-8 transform -rotate-90">
                            ...
                            <circle
                              cx="16" cy="16" r="14" stroke="currentColor" strokeWidth="2" fill="none"
                              strokeDasharray={`${2 * Math.PI * 14}`}
                              strokeDashoffset={`${2 * Math.PI * 14 * (1 - Math.min(characterCount, maxLength) / maxLength)}`}
