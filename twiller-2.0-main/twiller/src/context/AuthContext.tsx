  ...
  useEffect(() => {
    ...
    const unsubcribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser?.email) {
        try {
          const res = await axios.get("http://localhost:5000/loggeduser", {
    ...
    try {
      ...
      try {
        ...
        const newuser: any = {
          ...
          displayName: firebaseuser.displayName || "Twitter User",
          avatar: firebaseuser.photoURL || "https://github.com/shadcn.png",
          email: firebaseuser.email,
          username:firebaseuser.email.split("@")[0],
        };

        const registerRes = await axios.post("http://localhost:5000/register", newuser);
